...
public class DrinkingC2SPacket {
    ...
    private static final String MESSAGE_DRINKING_WATER = "message.random.drank_water";
    private static final String MESAGE_NO_WATER = "message.random.nowater";

    public static void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler,
                               PacketByteBuf buf, PacketSender responseSender) {
        ServerWorld world = player.getWorld();
        if(isAroundWaterThem(player, world, 2)) {
            player.sendMessage(Text.translatable(MESSAGE_DRINKING_WATER).fillStyle(Style
                    .EMPTY.withColor(Formatting.DARK_AQUA)));

            world.playSound(null, player.getBlockPos(), SoundEvents.ENTITY_GENERIC_DRINK, SoundCategory.PLAYERS,
                    0.5f, world.random.nextFloat() * 0.F + 0.9F);
        } else {
            player.sendMessage(Text.translatable(MESAGE_NO_WATER).fillStyle(Style
                    .EMPTY.withColor(Formatting.RED)));
        }
